...
/**
  ...
  * @return void
  */
void free_markov_node(MarkovNode* m_node);
...
MarkovNode* get_first_random_node(MarkovChain *markov_chain)
{
    ...
    while (bad_string)
    {
        ...
        int data_length = strlen(m_node->data) - 1;
        if (m_node->data[data_length] != '.')
        {
            return m_node;
        }
        rand_number =
...
bool add_node_to_counter_list(MarkovNode *first_node, MarkovNode *second_node)
{
    ...
    {
        if((n_n_c_list+index)->node_markov == second_node)
        {
...
Node* add_to_database(MarkovChain *markov_chain, char *data_ptr)
{
   ...
   if(result != NULL)
   {
    ...
    int word_len = strlen(data_ptr) +1;
    LinkedList *p = markov_chain->database;

    char* word_p = malloc(word_len);
    if(!word_p)
    {
        return NULL;
    }
    memcpy(word_p , data_ptr, word_len * sizeof(char));
...
Node* get_node_from_database(MarkovChain *markov_chain, char *data_ptr)
{
    ...
    while(temp_node != NULL)
    {
        ...
        if(!strcmp(data_ptr,markov_node->data))
        {
            return temp_node;
        }
